...
void TrainBatch(LeNet5 *lenet, image *inputs, uint8 *labels, int batchSize, int rank, int size)
{
    int local_batch_size = batchSize / size;
    int start_index = rank * local_batch_size;
    int end_index = (rank == size - 1) ? batchSize : start_index + local_batch_size;
